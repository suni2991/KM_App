...
const EditNomination = () => {
  ...
  const fetchExistingDataForEditing = async () => {
    try {
      const response = await fetch(
        `http://localhost:6001/nomination/${email}`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        const lastTraining =
          nominationData.trainings[nominationData.trainings.length - 1] || {};
        ...
        const trainingTopics = nominationData.trainings.map(
  ...
  const handleSubmit = async (e) => {
    ...
    if (nominationData) {
      const existingTraining = nominationData.trainings.find(
        (training) =>
          training.date === isoDate &&
          training.trainingName === employeeData.trainingTopic
      );
      ...
      if (existingTraining) {
        Swal.fire({
          icon: "error",
          title: "Error!",
          text: "Training with the same date and topic already exists for this employee",
          showConfirmButton: true,
          confirmButtonColor: "#00B4D2",
        });
        return;
    ...
    try {
      const response = await fetch(
        `http://localhost:6001/nominations/${email}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleFormChange = (event) => {
    ...
    if (name === "trainingTopic") {
      if (existingTrainingTopics.includes(value)) {
    ...
    if (name === "fromTime" || name === "toTime") {
      setEmployeeData({
        ...employeeData,
        [name]: value,
        timeSlot: `${employeeData.fromTime} - ${employeeData.toTime}`, // Update timeSlot with custom time
      });
    }
  };
  ...
  return (
    <div className="reg-container">
      ...
      <form>
        ...
        <Dropdown
          apiUrl="http://localhost:6001/topics/Training" // Adjust the API URL accordingly
        ...
        {duplicateTopicError && (
          <p style={{ color: "red" }}>This training topic is already taken.</p>
        )}
        <br />
        ...
        <button className="send-button" onClick={handleRedirect} type="submit">
          BACK
        </button>
      </form>
    </div>
