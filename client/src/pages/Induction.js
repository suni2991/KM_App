...
function Induction() {
  ...
  useEffect(() => {
    const fetchTopicsForInduction = async () => {
      try {
        const response = await axios.get(
          "http://localhost:6001/topics/Induction"
        );
  ...
  const handleTopicChange = (event) => {
    ...
    setSelectedTopic(selectedTopicName); // Set the selected topic name
  };
  ...
  const updateTopicsAndPresenters = async () => {
    try {
      if (!selectedTopic || !newPresenter) {
        Swal.fire({
          icon: "error",
          title: "Please select a topic and enter a presenter",
      ...
      const res = await fetch("http://localhost:6001/employees/update", {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      if (res.status === 200) {
        setSelectedRows(updatedRows); // Update the state with new values
        closeModal(); // Close the modal after updating
        Swal.fire({
          icon: "success",
          title: "Topics and presenters updated successfully",
          showConfirmButton: true,
          confirmButtonColor: "#00B4D2",
        });
        const updatedData = await fetchUsers(API);
        setData(updatedData);
      } else {
        Swal.fire({
          icon: "error",
          title: "Failed to update topics and presenters",
          showConfirmButton: true,
          confirmButtonColor: "#00B4D2",
        });
      }
  ...
  const sendEmail = async (rowData) => {
    ...
    // Filter topics where inductionStatus is 'Not Received'
    const notReceivedTopics = topics.filter(
      (topic) => topic.inductionStatus === "Not Received"
    );

    // Extract topics and presenters from filtered topics
    const topicsList = notReceivedTopics.map((topic) => ({
      topic: topic.topic,
      presenter: topic.presenter,
    }));

    // Construct email payload with topicsList
    const emailPayload = {
      email,
      fullName,
      confirmPassword,
    ...
    try {
      ...
      if (data.status === 401 || !data) {
        ...
        // Update employee status after successful email sending
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => {
        // Extract the last added topic from the topics array
        const lastAddedPresenter =
          row.topics.length > 0
            ? row.topics[row.topics.length - 1].presenter
  ...
  return (
    <div className="table-container">
      <h1 style={{ color: "#00B4D2", fontWeight: "bold" }}>
        {" "}
        Welcome to the Induction Dashboard{" "}
      </h1>
      ...
      <div className="search-filter2">
        ...
        <button
          ...
          title="send Email"
          style={{ visibility: "visible", color: "#fff" }}
      ...
      <center>
        <i>
          <p style={{ color: "#00B4D2", fontWeight: "bold" }}>
            {" "}
            *Select an Employee and click on Send Email button to send their
      ...
      <div className="modal" style={{ display: modalOpen ? "block" : "none" }}>
        <div className="modal-content">
          ...
          <h1 style={{ color: "rgb(8, 8, 68)", fontWeight: "bold" }}>
            UPDATE TOPIC - PRESENTER
          </h1>
          <br />
          <label
            htmlFor="topic"
            style={{ fontWeight: "bold", color: "#00B4D2" }}
          >
            Topic:
          </label>
          <br />
          <select
            ...
            id="induction-topic-select"
            value={selectedTopic} // Use selectedTopic (name) here
          ...
          <button className="send-button" onClick={updateTopicsAndPresenters}>
            Update
          </button>
          <center>
            <p style={{ color: "#00B4D2" }}>
              Will be updated to all selected employees
            </p>
          </center>
        </div>
      </div>
