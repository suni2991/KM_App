  ...
  const handleDownload = () => {
    ...
    const formattedData = filteredData.map((row) => {
      const feedbacksData = row.feedbacks.map((feedback, index) => ({
        ...
        "4. Did the trainer effectively engage you and create a supportive and inclusive learning environment?":
          feedback.question4,
        "5. Do you feel that the training helped you enhance your understanding and knowledge?":
          feedback.question5,
        "6. Was the session conducted in a professional manner?":
          feedback.question6,

        "7.  Do you have any suggestions for enhancing the training experience?":
          feedback.comment,
      }));
          ...
          <div style={{ display: "flex", flexWrap: "wrap" }}>
            {row.feedbacks.slice(-3).map((feedback, index) => (
              <button
                key={index}
                className={
                  expandedFeedbacks[row._id]?.[feedback._id]
            ...
            row.feedbacks.slice(-3).map((feedback, index) => (
              <div
                ...
                style={{
                  marginBottom: "5px",
                  textAlign: "left",
                  display: expandedFeedbacks[row._id][feedback._id]
                    ? "block"
                    : "none",
                }}
              >
                <div style={{ fontWeight: "bold" }}>
                  Presenter:{feedback.presenter}-{feedback.feedbackFor}
                </div>
                <div>
                  <strong>
                ...
                <div>
                  <strong>3. Was the trainer easy to understand?: </strong>{" "}
                  {feedback.question3}
                </div>
                <div>
                  <strong>
      ...
      <center>
        <i>
          <p style={{ color: "#00B4D2", fontWeight: "bold" }}>
            {" "}
            *Displaying the most recent induction details, including up to 3
            feedbacks
          </p>
        </i>
      </center>
