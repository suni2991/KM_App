...
const HistoryModal = ({ visible, onCancel, data }) => {
	const columns = [
		...
		},
		{
			...
			dataIndex: "comment",
			key: "comment",
			render: (text, record) => text && text.trim() ? record.comment : "Date Assigned"
		},
		{
			title: "Assigned At",
			dataIndex: "date",
			...
			render: (text) => {					//format(new Date(text), "MM/dd/yyyy HH:mm:ss"),
				if (!text) return "N/A"; 			// Handle null or undefined
				const parsedDate = new Date(text);
				return isNaN(parsedDate) ? "Invalid Date" : format(parsedDate, "MM/dd/yyyy hh:mm:ss a");
			}
		},
		//{
		//	title: "Date Assigned",
