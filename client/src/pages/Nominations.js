  ...
  const fetchNominations = async () => {
    try {
      const response = await fetch(
        `http://localhost:6001/nominations/${auth.email}`,
        {
          headers: {
  ...
  const handleDelete = async (row) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(
            `http://localhost:6001/nominations/${row._id}`,
            {
              method: "DELETE",
              headers: {
                Authorization: `Bearer ${token}`,
        ...
        Swal.fire("Cancelled", "Aborted", "info");
      }
    });
  };
  ...
  return (
    <div className="table-container">
      <h1 style={{ color: "rgb(0, 180, 210)", fontWeight: "bold" }}>
        {" "}
        Welcome to the Training Nominations Dashboard{" "}
      </h1>
